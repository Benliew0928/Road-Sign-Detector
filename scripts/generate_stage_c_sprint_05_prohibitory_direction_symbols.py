...
STAGE_ID = "stage_c_sprint_05_prohibitory_direction_symbols"
OUTPUT_ROOT = PROJECT_ROOT / "data/generated/stage_c_sprint_05_prohibitory_direction_symbols"
MANIFEST_PATH = PROJECT_ROOT / "data/manifests/stage_c_sprint_05_prohibitory_direction_symbols.csv"
REPORT_PATH = PROJECT_ROOT / "outputs/audit/stage_c_sprint_05_prohibitory_direction_symbols.json"
SHEET_ROOT = PROJECT_ROOT / "outputs/review/stage_c_sprint_05_prohibitory_direction_symbols"

CLASSES = {
    "no_straight_or_left": "straight_or_left",
    "no_straight_or_right": "straight_or_right",
    "no_left_or_right_turn": "left_or_right",
}
...
def draw_inner_symbol(draw: ImageDraw.ImageDraw, symbol: str) -> None:
    black = (20, 20, 20, 255)
    if symbol == "straight_or_left":
        draw_arrow(draw, (286, 362), (286, 130), fill=black, width=34)
        draw_arrow(draw, (286, 286), (150, 188), fill=black, width=34)
    elif symbol == "straight_or_right":
        draw_arrow(draw, (226, 362), (226, 130), fill=black, width=34)
        draw_arrow(draw, (226, 286), (362, 188), fill=black, width=34)
    elif symbol == "left_or_right":
        draw_arrow(draw, (258, 260), (136, 260), fill=black, width=34)
        draw_arrow(draw, (254, 260), (376, 260), fill=black, width=34)
    else:
        raise ValueError(f"Unsupported symbol: {symbol}")


def draw_prohibitory_symbol(symbol: str, *, red: tuple[int, int, int]) -> Image.Image:
    image = Image.new("RGBA", (DRAW_SIZE, DRAW_SIZE), (0, 0, 0, 0))
    draw = ImageDraw.Draw(image)
    outer = (42, 42, DRAW_SIZE - 42, DRAW_SIZE - 42)
    inner = (82, 82, DRAW_SIZE - 82, DRAW_SIZE - 82)
    draw.ellipse(outer, fill=(*red, 255))
    draw.ellipse(inner, fill=(248, 248, 242, 255))
    draw_inner_symbol(draw, symbol)
...
def make_background(rng: random.Random) -> Image.Image:
    palette = rng.choice(
        [
            ((154, 158, 150), (112, 122, 115)),
            ((184, 178, 166), (132, 126, 114)),
            ((126, 144, 156), (86, 103, 114)),
            ((142, 159, 135), (104, 118, 100)),
        ]
    )
...
def augment_symbol(semantic_id: str, symbol: str, variant_index: int) -> tuple[Image.Image, str]:
    ...
    sign = draw_prohibitory_symbol(symbol, red=red)
    ...
    summary = (
        f"symbol={symbol}; size={target_size}; angle={angle:.2f}; red={red}; "
...
def main() -> None:
    ...
    for semantic_id, symbol in CLASSES.items():
        for variant_index in range(1, VARIANTS_PER_CLASS + 1):
            synthetic_id = f"S05-{counter:03d}_{semantic_id}"
            ...
            rows.append(
                {
                    ...
                    "symbol_family": "prohibitory_direction",
    ...
    report = {
        ...
        "important_note": (
            "These generated candidates close prohibitory direction gaps at candidate-count "
